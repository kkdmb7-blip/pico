(function() {
  ...
  var FLOAT_ILJI = {
    good: {
      wood: ['{na}, 오늘 목기운 넘쳐요~ 🌿','창의적인 아이디어 잘 풀려요 🌱','뭐든 시작하기 딱이에요 🌿','{name}님 성장하는 날! 🌱','계획 실행하기 좋아요 🌳','뿌리내리기 좋은 날~ 🍃'],
      fire: ['{na}, 화기운 폭발이에요! 🔥','열정 가득한 하루예요 ✨','앞에 나서기 딱이에요 🌟','{name}님 오늘 빛나요 🌟','도전하면 잘 풀려요 🔥','존재감 드러나는 날~ ✨'],
      earth: ['{na}, 토기운 안정적이에요 🪨','중요한 결정 해도 좋아요 🌾','믿음직스러운 하루예요 🌽','{name}님 든든한 날~ 🪨','재물운 좋은 날이에요 💰','탄탄하게 굳어가요 🌾'],
      metal: ['{na}, 금기운 번뜩여요! 💎','집중력 최상이에요 ⚡','판단력이 빛나는 날 🤍','{name}님 분석력 최고! 💎','계약·협상 딱이에요 ⚡','정확하고 빠른 결단! 🤍'],
      water: ['{na}, 수기운 넘쳐요~ 💧','직관이 예리한 날이에요 🌊','{name}님 감 잘 맞아요 🫧','유연하게 흐르는 날~ 💧','수기운이 열어줘요 🌊','저 오늘 진짜 강해요! 🫧'],
    },
    bad: {
      wood: ['{na}, 목기운 약해요... 🍂','새 시작은 내일로 미뤄요 🌧️','자책하지 말아요~ 🌿','{name}님 조용히 버텨요 🍂','오늘은 쉬어가요 🌧️','무리하면 흔들려요 🍂'],
      fire: ['{na}, 화기운 꺼져가요 🕯️','감정 소모 줄여요 🌫️','논쟁은 피해요~ 🕯️','{name}님 천천히 가요 💤','쉬어가는 날이에요 🌫️','흥분하면 지쳐요 🕯️'],
      earth: ['{na}, 토기운 흔들려요 🪨','계약은 내일로 미뤄요 🌾','제가 옆에 있을게요 🤲','{name}님 중심 잡아요 🪨','투자·거래는 피해요 🌾','조용히 기다려요~ 🪨'],
      metal: ['{na}, 금기운 녹슬었어요 💔','예민해지기 쉬워요 ⛅','내일 해도 돼요~ 🤍','{name}님 말 아껴요 💎','관찰하는 날로 해요 ⛅','감정적 충돌 조심! 💎'],
      water: ['{na}, 수기운 탁해요 🌊','감정 기복 조심해요 🌧️','저도 쳐져있어요... 💧','{name}님 조용히 있어요 🌊','충동 결정 주의! 💧','억지로 밀지 말아요 🌧️'],
    },
    normal: {
      wood: ['{na}, 목기운 무난해요 🌱','꾸준히 하면 충분해요 🌿','{name}님 루틴대로~ 🌳','차분하게 가요 🌱'],
      fire: ['{na}, 화기운 안정적이에요 🕯️','평소대로 하면 돼요 ✨','{name}님 한 가지씩~ 🔥','무리 안 하면 괜찮아요 🕯️'],
      earth: ['{na}, 토기운 무난해요 🌾','편하게 보내요~ 🪨','{name}님 내실 다져요 🌽','조용한 하루예요 🌾'],
      metal: ['{na}, 금기운 평범해요 💎','무리 없이 보내요 ⚡','{name}님 하나씩~ 🤍','급한 것만 챙겨요 💎'],
      water: ['{na}, 수기운 잔잔해요 💧','흐름 타면서 가요~ 🌊','{name}님 차분하게 🫧','천천히 가도 돼요 💧'],
    },
  };
  ...
  // ── 사용자 프로필 출생일 기반 개인화 ──
  // localStorage.pico_profile {name, year, month, day, hour, gender}
  function getUserProfile() {
    try { return JSON.parse(localStorage.getItem('pico_profile') || 'null'); }
    catch(e) { return null; }
  }
  function getUserAgeDays() {
    var p = getUserProfile();
    if (!p || !p.year || !p.month || !p.day) return -1;
    var bd = new Date(Number(p.year), Number(p.month) - 1, Number(p.day));
    if (isNaN(bd.getTime())) return -1;
    var now = new Date();
    var today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    var diff = Math.floor((today - bd) / 86400000);
    return diff >= 0 ? diff : -1;
  }
  function userAgeGreet() {
    var d = getUserAgeDays();
    if (d < 0) return null;
    var p = getUserProfile();
    var who = (p && p.name) ? p.name + '님' : '당신';
    if (d === 0)      return who + '이(가) 오늘 세상에 나왔어요 🐣';
    if (d === 1)      return who + '이(가) 세상에 나온 지 하루 됐어요';
    if (d === 100)    return who + '이(가) 세상에 나온 지 100일 🌸';
    if (d === 365)    return who + '이(가) 세상에 나온 지 1년 💖';
    if (d === 1000)   return who + '이(가) 세상에 나온 지 1000일 ✨';
    if (d === 10000)  return who + '이(가) 세상에 나온 지 1만일 🎉';
    if (d > 0 && d % 1000 === 0) return who + '이(가) 세상에 나온 지 ' + d.toLocaleString() + '일 ✨';
    if (d > 0 && d % 365 === 0)  return who + '이(가) 세상에 나온 지 ' + (d/365) + '년이 되었어요 🌿';
    return who + '이(가) 세상에 나온 지 ' + d.toLocaleString() + '일 되었어요';
  }
  ...
  function startProactiveGreeter() {
    ...
    setInterval(function() {
      ...
      var uAge = userAgeGreet();
      if (uAge) {
        pool.push(uAge);
        var ud = getUserAgeDays();
        if (ud === 100 || ud === 365 || ud === 1000 || ud === 10000 ||
            (ud > 0 && (ud % 1000 === 0 || ud % 365 === 0))) {
          pool.push(uAge, uAge);
        }
      }
      var persona = getPersona();
